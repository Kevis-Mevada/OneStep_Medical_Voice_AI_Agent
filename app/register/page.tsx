...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (credential.user) {
        // Send verification email using our helper function
        const emailResult = await sendVerificationEmail(credential.user);
        ...
        if (emailResult.success) {
          ...
          setError(emailResult.error || "Account created but failed to send verification email. Please try logging in.");
        }
      }
    } catch (err: any) {
      console.error(err);
      ...
      if (err?.code === "auth/email-already-in-use") {
        ...
        setError("Password is too weak. Please use at least 8 characters with mixed case and numbers.");
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (err?.code === "auth/popup-closed-by-user") {
        setError("Sign-up window was closed. Please click the button again to continue.");
      } else if (err?.code === "auth/cancelled-popup-request") {
        // User closed popup or opened another one - silently ignore
        setError(null);
      } else if (err?.message?.includes('timeout')) {
        setError("Sign-up took too long. Please try again.");
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#F8FAFC] px-4">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>
            Register to start safe, non-diagnostic health consultations. You&apos;ll need to verify your email before using the assistant.
          </CardDescription>
        </CardHeader>
